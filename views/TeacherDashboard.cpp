...
void TeacherDashboard::addNewClass()
{
    ...
    if (!ok || classCode.isEmpty()) return;
...
void TeacherDashboard::populateSubjectCombo()
{
    ...
    for (int i = 0; i < danhSachMon.size(); i++) {
        MonHoc* mon = danhSachMon.get(i);
        QString item = QString("%1 - %2").arg(QString::fromStdString(mon->getMaMon()))
...
void TeacherDashboard::editClass() {
    ...
    if (!ok || newClassCode.isEmpty()) return;
    ...
    if (!ok || newClassName.isEmpty()) return;
...
void TeacherDashboard::editStudent() {
    ...
    if (!ok || newFirstName.isEmpty()) return;
...
void TeacherDashboard::deleteStudent() {
    ...
    QString studentName = QString("%1 %2").arg(studentTable->item(row, 1)->text())
                                         .arg(studentTable->item(row, 2)->text());
...
void TeacherDashboard::editSubject() {
    ...
    if (!ok || newSubjectCode.isEmpty()) return;
    ...
    if (!ok || newSubjectName.isEmpty()) return;
...
void TeacherDashboard::deleteSubject() {
    ...
    int result = QMessageBox::question(this, "Confirm Delete", 
                                     QString("Are you sure you want to delete subject '%1 - %2'?\n\n"
                                            ...
                                            "This action cannot be undone.")
                                     .arg(subjectCode).arg(subjectName),
                                     QMessageBox::Yes | QMessageBox::No, 
                                     QMessageBox::No);
...
void TeacherDashboard::editQuestion() {
    ...
    if (currentCorrect == 'A') correctCombo->setCurrentIndex(0);
    else if (currentCorrect == 'B') correctCombo->setCurrentIndex(1);
    else if (currentCorrect == 'C') correctCombo->setCurrentIndex(2);
    else if (currentCorrect == 'D') correctCombo->setCurrentIndex(3);
...
{
    if (!classManager || !subjectManager) return;
    ...
    for (int i = 0; i < danhSachLop.size(); i++) {
        Lop* lop = danhSachLop.get(i);
        QString item = QString("%1 - %2").arg(QString::fromStdString(lop->getMaLop()))
                                         .arg(QString::fromStdString(lop->getTenLop()));
