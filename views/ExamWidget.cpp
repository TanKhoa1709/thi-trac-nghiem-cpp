...
void ExamWidget::updateQuestionNavigation()
{
    ...
    while (navLayout->count() > 2) { // Keep title and stretch
...
void ExamWidget::updateProgress()
{
    ...
    progressBar->setFormat(QString("Progress: %1/%2 questions answered (%p%)")
                          .arg(answeredQuestions).arg(questions->size()));
}
...
{
    if (optionA->isChecked()) return 'A';
    if (optionB->isChecked()) return 'B';
    if (optionC->isChecked()) return 'C';
    if (optionD->isChecked()) return 'D';
